...
void exit_error1(const char *what)
{
   printf("ERROR: %s\n", what);
   exit(1);
}
...
int main(int argc, char *argv[])
{
    ...
    if ((fin = fopen(argv[1], "rb")) == NULL) exit_error1("source file");

    if ((fout = fopen(argv[2], "wb")) == NULL) exit_error1("destination file");
    ...
    if (source == NULL) exit_error1("source memory");

    if (fread(source, 1, len, fin) != len) exit_error1("read");
	...
	if (comp.hash_table == NULL) exit_error1("table memory");
